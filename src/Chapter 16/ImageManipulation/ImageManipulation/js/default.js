(function () {
    ...
    function showProgress(id, show) {
        var progress = progress = document.getElementById(id);
        ...
        picker.pickSingleFileAsync().then(function (file) {
            if (file) {
                ...
                file.getThumbnailAsync(Windows.Storage.FileProperties.ThumbnailMode.picturesView).done(function (thumb) {
                    ...
                    document.getElementById("thumb1").src = window.URL.createObjectURL(stream);
    ...
    //Convert pixels to grayscale (using one of differnet implementations), and render to the canvas
    function copyGrayscaleToCanvas(pixels, width, height, componentType) {
        ...
        switch (componentType) {
            ...
            case "CS":
                var option = 1;  //Synchronous method by default
